  ...
  <div class="col-lg-12 col-md-12 col-sm-12">
    ...
    <b-table hover :items="items" :fields="fields" v-if="items.length != 0">
      <template v-slot:cell(action)="data">
        ...
        <router-link
          :to="{ name: 'ProductDetail', params: { url: data.item.url } }"
    ...
    checkFormValidity() {
      ...
      handleSubmit() {
        ...
        if (!this.checkFormValidity()) {
          ...
          this.axios
          .put("http://127.0.0.1:8000/api/category/"+this.updatecategory.id,this.updatecategory)
          .then(() => {
            this.$emit("render");
        ...
        // Ẩn model
        this.$nextTick(() => {
          this.$bvModal.hide('modal-prevent-sua')
        })   
      },
      updateCategory(id){
         this.axios
